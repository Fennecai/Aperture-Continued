...
function ENT:Think()
	...
	if not timer.Exists("TA:Monsterbox_Chitter" .. self:EntIndex()) then
		timer.Create(
			"TA:Monsterbox_Chitter" .. self:EntIndex(),
			math.Rand(1, 3),
			1,
			function()
			end
	...
	if
		self:IsPlayerHolding() or self.Cubemode or
			(IsValid(traceDown.Entity) and
				(traceDown.Entity:GetClass() == "sent_portalbutton_box" or traceDown.Entity:GetClass() == "sent_portalbutton_normal" or
					traceDown.Entity:GetClass() == "sent_portalbutton_old" or
					traceDown.Entity:GetClass() == "portalbutton_phys"))
	 then
		if not timer.Exists("TA:Monsterbox_Hermit" .. self:EntIndex()) then
			timer.Create(
				"TA:Monsterbox_Hermit" .. self:EntIndex(),
				self:PlaySequence("hermit_idle", 1.0),
				1,
				function()
				end
			)
	...
	if not traceDown.Hit then
		...
		if not timer.Exists("TA:Monsterbox_Intheair" .. self:EntIndex()) then
			timer.Create(
				"TA:Monsterbox_Intheair" .. self:EntIndex(),
				self:PlaySequence("intheair", 1.0),
				1,
				function()
				end
			)
	...
	if not traceBottom.Hit then
		if not timer.Exists("TA:Monsterbox_Fallover" .. self:EntIndex()) then
			timer.Create(
				"TA:Monsterbox_Fallover" .. self:EntIndex(),
				self:PlaySequence("fallover_idle", 1.0),
				1,
				function()
				end
			)
		end
		if timer.Exists("TA:Monsterbox_Trapped" .. self:EntIndex()) then
		...
		if traceForward.Hit then
			if not timer.Exists("TA:Monsterbox_Trapped" .. self:EntIndex()) then
				timer.Create(
					"TA:Monsterbox_Trapped" .. self:EntIndex(),
					self:PlaySequence("trapped", 1.0),
					0,
					function()
					end
				)
			...
			return true
		elseif timer.Exists("TA:Monsterbox_Trapped" .. self:EntIndex()) then
			timer.Remove("TA:Monsterbox_Trapped" .. self:EntIndex())
		end
	end
	...
	if not timer.Exists("TA:Monsterbox_Straight" .. self:EntIndex()) then
		...
		timer.Create(
			"TA:Monsterbox_Straight" .. self:EntIndex(),
			self:PlaySequence("straight0" .. animType, 1.0),
			1,
			function()
			end
		)
...
function ENT:Jump(force, forceUp)
	...
	if timer.Exists("TA:Monsterbox_Trapped" .. self:EntIndex()) or self:IsPlayerHolding() then
		return
	end
	...
	timer.Simple(
		...
		function()
			if IsValid(self) then
				self:EmitSound("TA:MonsterBoxFootsteps")
			end
		end
	)
end
