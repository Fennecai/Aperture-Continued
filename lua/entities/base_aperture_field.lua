...
if SERVER then
	function ENT:CreateSecondEmitter()
		...
		local trace =
			util.QuickTrace(
			self:GetPos(),
			-self:GetRight() * LIB_MATH_TA.HUGE,
			function()
			end
		)
		...
		local _,
			ang = LocalToWorld(Vector(), Angle(0, 180, 0), Vector(), angles)
...
function ENT:Think()
	if CLIENT then
		-- clearing
		if not self:GetEnable() then
			if self.FieldsEntities and #self.FieldsEntities > 0 then
				self:ClearFields()
			end
			return
		end
		...
		if self.FieldStrech then
			if #self.FieldsEntities ~= 1 then
				...
				if self.FieldMaterial then
					ent:SetSubMaterial(0, self.FieldMaterial)
				end
				ent:SetNoDraw(true)
	...
	if SERVER then
		...
		if not IsValid(triggerEnt) then
			return
		end
		local dist = self:GetPos():Distance(secondEmitter:GetPos())
		local vec1 = self:GetUp() * FIELD_MODEL_SIZE / 2 + self:GetForward() * FIELD_TRIGGER_THICK - self:GetRight() * dist
