...
if SERVER then
	function ENT:RemoveTrails()
		if IsValid(self.TA_Trail1) then
			self.TA_Trail1:Remove()
		end
		if IsValid(self.TA_Trail2) then
			self.TA_Trail2:Remove()
		end
		if IsValid(self.TA_Trail3) then
			self.TA_Trail3:Remove()
		end
	end
			...
			function()
				...
				if enable then
					...
					self.TA_Trail1 =
						...
						trailWidthEnd,
						1,
						1 / (trailWidth + trailWidthEnd) * 0.5,
						material
					)
					self.TA_Trail2 =
						util.SpriteTrail(
...
function ENT:Drawing()
	...
	local material =
		reverse and Material("effects/particle_ring_pulled_add_oriented_reverse") or
		Material("effects/particle_ring_pulled_add_oriented")
	...
	if not timer.Exists("TA:TractorBeamEffect" .. self:EntIndex()) then
		...
		timer.Create(
			"TA:TractorBeamEffect" .. self:EntIndex(),
			0.05,
			1,
			function()
			end
...
if CLIENT then
	function ENT:Think()
		...
		local passagesPoints =
			LIB_APERTURECONTINUED:GetAllPortalPassagesAng(
			self:GetPos(),
			self:LocalToWorldAngles(Angle(-90, 0, 0)),
			nil,
			self,
			true
		)
		...
		for k, v in pairs(passagesPoints) do
			...
			if penetrateVal > FUNNEL_EFFECT_MODEL_SIZE then
				penetrateVal = penetrateVal - FUNNEL_EFFECT_MODEL_SIZE
			end
			if penetrateVal < 0 then
				penetrateVal = penetrateVal + FUNNEL_EFFECT_MODEL_SIZE
			end
		end
		...
		if self:GetEnable() then
			...
			for k, v in pairs(passagesPoints) do
				local direction = (v.endpos - v.startpos):GetNormalized()
				local _,
					angles = LocalToWorld(Vector(), Angle(angle, 0, 0), Vector(), v.angles)
				local distance = v.startpos:Distance(v.endpos)
				local offsetV = penetrateVal == 0 and 0 or FUNNEL_EFFECT_MODEL_SIZE - penetrateVal
			...
			if self.BaseRotation > 360 then
				self.BaseRotation = self.BaseRotation - 360
			end
			if self.BaseRotation < -360 then
				self.BaseRotation = self.BaseRotation + 360
			end
...
function ENT:HandleEntity(ent, beamStart, beamEnd, beamAng, beamDirection, isLast)
	...
	if self.EntitiesInFunnel[ent:EntIndex()] then
		self.EntitiesInFunnel[ent:EntIndex()] = nil
	end

	local centerPos =
		IsValid(ent:GetPhysicsObject()) and ent:LocalToWorld(ent:GetPhysicsObject():GetMassCenter()) or ent:GetPos()
	...
	if ent:IsPlayer() then
		...
		if ent:KeyDown(IN_FORWARD) then
			movingDir = movingDir + Vector(1, 0, 0)
		end
		if ent:KeyDown(IN_BACK) then
			movingDir = movingDir - Vector(1, 0, 0)
		end
		if ent:KeyDown(IN_MOVELEFT) then
			movingDir = movingDir + Vector(0, 1, 0)
		end
		...
		if ent:KeyDown(IN_FORWARD) or ent:KeyDown(IN_BACK) or ent:KeyDown(IN_MOVERIGHT) or ent:KeyDown(IN_MOVELEFT) then
...
function ENT:CheckForLeave()
	if not self.EntitiesInFunnel then
		return
	end
		...
		util.TraceHull(
			{
				...
				filter = function(ent)
					if ent ~= self and ent:GetClass() ~= "prop_portal" then
						if
							not ent:IsPlayer() and not ent:IsNPC() and IsValid(ent:GetPhysicsObject()) and
								not ent:GetPhysicsObject():IsMotionEnabled()
						 then
						else
							local isLast = false
...
function ENT:TriggerInput(iname, value)
	...
	if iname == "Enable" then
		self:EnableEX(tobool(value))
	end
	if iname == "Reverse" then
		self:ReverseEX(tobool(value))
	end
end
