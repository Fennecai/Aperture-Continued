...
function ENT:SetupDataTables()
	...
	self:NetworkVar("Float", 3, "DropForce")
end

function ENT:ModelToInfo()
	local modelToInfo = {
		["models/aperture/item_dropper.mdl"] = {
			open = "item_dropper_open",
			close = "item_dropper_close",
			opensound = "TA:ItemDropperOpen",
			closesound = "TA:ItemDropperClose",
			blockerZ = 45
		},
		["models/aperture/underground_boxdropper.mdl"] = {
			open = "open",
			close = "close",
			opensound = "TA:OldItemDropperOpen",
			closesound = "TA:OldItemDropperClose",
			blockerZ = 10
		}
	}
...
function ENT:Initialize()
	...
	if SERVER then
		...
		self.Inputs = Wire_CreateInputs(self, {"Drop", "Force"})
	end
...
function ENT:Think()
	...
	if not IsValid(self.ItemInDropper) then
		...
		if not IsValid(self.LastDroppedItem) then
...
function ENT:CreateItem()
	...
	if info.skin then
		item:SetSkin(info.skin)
	end
	if info.class == "prop_monster_box" then
		item.Cubemode = true
	end
	if info.class == "ent_portal_bomb" then
		item.BombDisabled = true
	end
...
function ENT:Drop(shoulddrop)
	...
	if IsValid(self.LastDroppedItem) and self.LastDroppedItem ~= self.ItemInDropper then
		LIB_APERTURECONTINUED:DissolveEnt(self.LastDroppedItem)
	end
	...
	timer.Simple(
		...
		function()
			...
			if not IsValid(lastSpawnedItem) then
				return
			end
			local lastSpawnedItemPhys = lastSpawnedItem:GetPhysicsObject()
			if not IsValid(lastSpawnedItemPhys) then
				return
			end
			...
			if lastSpawnedItem:GetClass() == "ent_portal_bomb" then
				lastSpawnedItem.BombDisabled = false
			end
	...
	-- Spawn new item
	timer.Create(
		"TA:ItemDroper_Redrop" .. self:EntIndex(),
		2.5,
		1,
		function()
			if not IsValid(self) then
	...
	timer.Create(
		"TA:ItemDroper_CanDrop" .. self:EntIndex(),
		3.5,
		1,
		function()
			if IsValid(self) then
...
function ENT:TriggerInput(iname, value)
	...
	if iname == "Force" then
		print("wiremod input now called on the dropper, value is" .. value)
		self.dropforcevar = value
	end
end

function ENT:OnRemove()
	if IsValid(self.LastDroppedItem) then
		self.LastDroppedItem:Remove()
	end
	if IsValid(self.ItemInDropper) then
		self.ItemInDropper:Remove()
	end
end
