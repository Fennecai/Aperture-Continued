...
local function FilterValid(pipe, ent, routeInx)
     if not pipe.DivventFilter then
          return true
     end
     if not pipe.DivventFilter[routeInx] then
          return true
     end
     ...
     for k, v in pairs(filters) do
          ...
          if filterType == 1 then
               if ent:GetModel() == string.lower(text) then
                    return true
               end
          elseif filterType == 2 then
               if ent:GetClass() == text then
                    return true
               end
          elseif filterType == 3 then
               ...
               if IsValid(physObj) then
                    ...
                    if mass >= min and mass < max then
                         return true
                    end
               end
          elseif filterType == 4 then
               local radius = ent:GetModelRadius()
               if radius >= min and radius < max then
                    return true
               end
...
function ENT:HandleEntities(ent, portalable, fromportal)
     ...
     if self:GetIgnoreAlive() and (ent:IsPlayer() or ent:IsNPC()) then
          return
     end
     if LIB_APERTURECONTINUED.DIVVENT_ENTITIES[ent] then
          return
     end
     if not IsValid(self:GetNWEntity("TA:ConnectedPipe:1")) then
          return
     end
