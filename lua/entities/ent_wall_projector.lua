...
function ENT:Think()
	...
	for k, v in pairs(passagesPoints) do
		...
		if penetrateVal > WALL_MODEL_SIZE then
			penetrateVal = penetrateVal - WALL_MODEL_SIZE
		end
	...
	if #self.ProjectedWalls ~= requireToSpawn or not self:GetEnable() then
		for k, v in pairs(self.ProjectedWalls) do
			v:Remove()
		end
		self.ProjectedWalls = {}
	end
...
function ENT:Drawing()
	...
	for k, v in pairs(passagesPoints) do
		...
		render.DrawBeam(
			...
			30,
			1,
			v.startpos:Distance(v.endpos) / 50,
			Color(255, 255, 255)
		)
		render.DrawBeam(
			v.startpos - offset,
