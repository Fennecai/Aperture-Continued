...
function ENT:Drawing()
	...
	if not IsValid(secondEmitter) then
		return
	end
	...
	local tracer =
		util.TraceHull(
		{
			...
			filter = function(ent)
				if not ent.IsAperture and ent ~= self and ent ~= secondEmitter and not ent:IsPlayer() and not ent:IsNPC() then
	...
	for _, ent in pairs(closesEntities) do
		...
		local color =
			Color(
			math.min(self.FieldColor.r * flash, 255),
			math.min(self.FieldColor.g * flash, 255),
			math.min(self.FieldColor.b * flash, 255)
		)
		color.a = alpha
...
function ENT:HandleEntity(ent)
	...
	if ent.InUnFizzable then
		return
	end
	if ent == self:GetSecondEmitter() then
		return
	end
