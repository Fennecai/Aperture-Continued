...
function ENT:TransformPlate()
	...
	if self.SmashStartTime then
		...
		if time <= 0.6 then
			plate:SetPos(self:LocalToWorld(Vector((time / 0.6) * length, 0, 0)))
		elseif time <= 1.6 then
			plate:SetPos(self:LocalToWorld(Vector(length, 0, 0)))
...
function ENT:DamageEntities()
	...
	for k, v in pairs(entities) do
		if LIB_APERTURECONTINUED:IsValidHealthEntity(v) then
			v:TakeDamage(250, self, self)
		end
...
function ENT:StartSmash()
	...
	timer.Simple(
		plate:PlaySequence("smash", 1.0) + 0.25,
		function()
			if IsValid(self) then
				self:SetBusy(false)
			end
