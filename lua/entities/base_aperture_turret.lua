...
function ENT:Drawing()
	...
	if not self.TurretDrawLaserbeam then
		return
	end
	if self:GetNWBool("TA:TurretDifferent") then
		return
	end
...
if SERVER then
	...
	function ENT:Shoot(angle)
		if timer.Exists("TA:TurretShooting" .. self:EntIndex()) then
			return
		end
		if self.CantShoot then
			timer.Create(
				"TA:TurretShooting" .. self:EntIndex(),
				2,
				1,
				function()
				end
...
function ENT:Think()
	...
	if CLIENT then
		...
		if turretsBone then
			self:ManipulateBoneAngles(turretsBone, angle)
		end
		-- wings
		if turretLeftBone then
			self:ManipulateBonePosition(turretLeftBone, Vector(-turretOpen, 0, 0) * 8)
		end
		if turretRightBone then
			self:ManipulateBonePosition(turretRightBone, Vector(turretOpen, 0, 0) * 8)
		end
		-- antenna
		if turretAntenna then
			self:ManipulateBonePosition(turretAntenna, Vector(0, turretOpen, 0) * 15)
		end
		-- guns
		if turretRGun1 then
			self:ManipulateBonePosition(turretRGun1, Vector(0, 0, turretOpen) * 4)
		end
		if turretRGun2 then
			self:ManipulateBonePosition(turretRGun2, Vector(0, 0, turretOpen) * 4)
		end
		if turretLGun1 then
			self:ManipulateBonePosition(turretLGun1, Vector(0, 0, turretOpen) * 4)
		end
		if turretLGun2 then
			self:ManipulateBonePosition(turretLGun2, Vector(0, 0, turretOpen) * 4)
		end
	...
	if self:GetEnable() then
		target,
			center =
			LIB_APERTURECONTINUED:FindClosestAliveInConeIncludingPortalPassages(
			eyePos,
			self:GetForward(),
			2000,
			TURRET_TARGET_DEGRESE
		)
	end
	if IsValid(target) then
		local _,
			trace = LIB_APERTURECONTINUED:GetAllPortalPassages(eyePos, (center - eyePos), nil, self)
		if not IsValid(trace.Entity) or trace.Entity ~= target then
			target = nil
		end
		if self.TurretDifferent then
			target = nil
		end
	end

	if
		(not self:GetEnable() and not self:IsTurretKnockout() or self.TurretDifferent) and turretState == TURRET_STATE_SEARCH
	 then
		-- Retracting turret
	...
	if self:GetEnable() then
		...
		if self:IsPlayerHolding() then
			if turretState ~= TURRET_STATE_PANIC_LIGHTER and turretState ~= TURRET_STATE_SHOOT then
				...
				if self.TurretSoundPickup then
					self:EmitSound(self.TurretSoundPickup)
				end
		...
		if self:IsTurretKnockout() then
			if turretState ~= TURRET_STATE_PANIC then
				...
				timer.Create(
					"TA:KnockoutTime" .. self:EntIndex(),
					3,
					1,
					function()
						if self.TurretDisabled then
							self:EmitSound(self.TurretDisabled)
						end
	...
	if turretState == TURRET_STATE_PREPARE_DEPLOY then
		if not timer.Exists("TA:PrepareDeploy" .. self:EntIndex()) then
			timer.Create(
				"TA:PrepareDeploy" .. self:EntIndex(),
				1,
				1,
				function()
					if not IsValid(self) then
					...
					if self.TurretSoundFound ~= "" then
						self:EmitSound(self.TurretSoundFound)
					end
	...
	elseif turretState == TURRET_STATE_PREPARE_RETRACT then
		...
		if not timer.Exists("TA:PrepareRetract" .. self:EntIndex()) then
			timer.Create(
				"TA:PrepareRetract" .. self:EntIndex(),
				1.5,
				1,
				function()
					if not IsValid(self) then
					...
					if self.TurretRetract ~= "" then
						self:EmitSound(self.TurretRetract)
					end
					...
					timer.Simple(
						...
						function()
							...
							timer.Remove("TA:StopSearching" .. self:EntIndex())
							timer.Create(
								"TA:TurretAutoSearch" .. self:EntIndex(),
								3,
								1,
								function()
									if not IsValid(self) then
									...
									if self.TurretSoundAutoSearch ~= "" then
										self:EmitSound(self.TurretSoundAutoSearch)
									end
								end
							)
						end
	...
	elseif turretState == TURRET_STATE_SEARCH then
		...
		elseif not timer.Exists("TA:StopSearching" .. self:EntIndex()) then
			timer.Create(
				"TA:StopSearching" .. self:EntIndex(),
				5,
				1,
				function()
					if not IsValid(self) then
	...
	elseif turretState == TURRET_STATE_SHOOT then
		if IsValid(target) then
			...
			if not timer.Exists("TA:TurretReleaseTarget" .. self:EntIndex()) then
				timer.Create(
					"TA:TurretReleaseTarget" .. self:EntIndex(),
					0.25,
					1,
					function()
						if not IsValid(self) then
						...
						if self.TurretSoundSearch ~= "" then
							self:EmitSound(self.TurretSoundSearch)
						end
	...
	elseif turretState == TURRET_STATE_PANIC then
		...
		self:RotateTurret(Angle(pitch, yaw, 0), 10)
		self:CheckFoShoot()
		if turretOpen < 1 then
			self:SetTurretOpen(turretOpen + 2 * FrameTime())
		else
			self:SetTurretOpen(1)
		end
	elseif turretState == TURRET_STATE_PANIC_LIGHTER then
		local specCurTime = CurTime() * 2 + self:EntIndex() * 5
		local pitch = math.cos(specCurTime * 1.5 + self:EntIndex() * 10) * 30
		...
		if not timer.Exists("TA:TurretActivatePing" .. self:EntIndex()) then
			...
			timer.Create(
				"TA:TurretActivatePing" .. self:EntIndex(),
				1 + math.Rand(0, 1),
				1,
				function()
				end
	...
	elseif turretState == TURRET_STATE_RETRACT then
		local turretOpen = self:GetTurretOpen()
		if turretOpen > 0 then
			self:SetTurretOpen(turretOpen - 2 * FrameTime())
		else
			self:SetTurretOpen(0)
		end
	end
...
function ENT:OnFizzle()
	if self.TurretSoundFizzle ~= "" then
		self:EmitSound(self.TurretSoundFizzle)
	end
	self:Enable(false)
end
