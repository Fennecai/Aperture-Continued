...
if CLIENT then
	...
	language.Add("tool.aperture_arm_panel.desc", "Fully configurable. Infinitely variable. Aperture brand panels will assist your test subjects every step of the way.")
...
local function GetSpawnPos(pos, ang, ply, normal)
	...
	if not IsValid(armPanel) then
			...
			if math.abs(normal.z) == 1 then
				_, angle = WorldToLocal(Vector(), (pos - ply:GetPos()):Angle(), Vector(), ang)
				angle = Angle(0, angle.y + 180, 0)
				_, angle = LocalToWorld(Vector(), angle, Vector(), ang)
			end
			ang:Set(angle)
		return
	end
	...
	if armPos == gridPos then return 1 end
	return 0
end
...
function TOOL:LeftClick(trace)
	...
	undo.Create("Arm Panel")
		for k,v in pairs(armPanelTable) do undo.AddEntity(v) end
		undo.SetPlayer(ply)
	undo.Finish()
...
if CLIENT then
	function TOOL:MakeGhostEntityInx(inx, subinx)
		if not self.TA_GhostEntityArray then self.TA_GhostEntityArray = {} end
	...
	function TOOL:RemoveGhostEntitityInx(inx, subinx)
		if not self.TA_GhostEntityArray then return end
		local subinx = subinx and subinx or 0
		local inxName = inx.."_"..subinx
		local ent = self.TA_GhostEntityArray[inxName]
		if not IsValid(ent) then return end
...
function TOOL:TransformGhostArmPanel(ent)
	...
	if (d1 - DofLength1) > DofLength2Min then dof2 = math.min((d1 - DofLength1) + 0.01, DofLength2Max) end
	...
	if angle.yaw == 180 then angle = Angle(-angle.pitch + 180, 0, 0) end
...
function TOOL:UpdateGhostArmPanelInx(offsetx, offsety, inx, ply)
	...
	local ang = trace.HitNormal:Angle() + Angle(90, 0, 0)
	local result = GetSpawnPos(pos, ang, ply, trace.HitNormal)
	if result == 1 then ent:SetNoDraw(true) return end
...
function TOOL.BuildCPanel( CPanel )
	...
	CPanel:AddControl("CheckBox", {Label = "#tool.aperture_arm_panel.startenabled", Command = "aperture_arm_panel_startenabled", Help = true})
