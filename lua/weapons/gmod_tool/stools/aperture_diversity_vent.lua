...
local function GetClosestVentPoint(ply)
	...
	local ent, pointType, pos, ang, index
...
if SERVER then
	...
	duplicator.RegisterEntityClass("ent_diversity_vent", MakePortalDiversityVent, "pos", "ang", "ignorealive", "key_enable", "startenabled", "toggle", "data")
...
local function SendFilterDat(routeInx, filterType, ent, text, min, max)
	net.Start("TA:DivventFilterNetwork")
		...
		net.WriteEntity(ent)
		if filterType == 1 or filterType == 2 then net.WriteString(text) end
...
net.Receive("TA:DivventFilterNetwork", function()
	...
	if filterType == 1 or filterType == 2 then text = net.ReadString() end
	if filterType == 3 or filterType == 4 then
		min = net.ReadFloat()
		max = net.ReadFloat()
	end
	
	if not ent.DivventFilter then ent.DivventFilter = {} end
	if not ent.DivventFilter[routeInx] then ent.DivventFilter[routeInx] = {} end
...
local function RemoveFilter(routeInx, scroll, panel)
	...
	if not ply:GetTool() then return end
	local ent = ply:GetTool().SelectedVent
	if not IsValid(ent) then return end
	if not ent.DivventFilter then return end
	if not ent.DivventFilter[routeInx] then return end
...
local function CreateFilterSettings(model)
	...
	dButton.DoClick = function(self)
		...
		if not self.RouteInx or self.RouteInx == 0 then return end
	...
	dComboBox.OnSelect = function(panel, index, value)
		...
		if IsValid(panel.LastTypeSettingsPanel) then panel.LastTypeSettingsPanel:Remove() end
...
function TOOL:LeftClick(trace)
	...
	if IsValid(vent) and vpointType == POINT_TYPE_CONNECTION then
		if not IsValid(vent:GetNWEntity("TA:ConnectedPipe:"..vindex)) then
			...
			if not IsValid(ent) and not istable(ent) then return end
			
			undo.Create("Pneumatic Diversity Pipe")
				if istable(ent) then
					for k,v in pairs(ent) do undo.AddEntity(v) end
				else
					undo.AddEntity(ent)
				end
				undo.SetPlayer(ply)
...
function TOOL:UpdateGhostDiversityVent(ent, ply)
	...
	if vpointType == POINT_TYPE_FILTER then ent:SetNoDraw(true) return end
	ent:SetNoDraw(false)
...
function TOOL:Think()
	local weapon = self:GetWeapon()
	if not weapon:GetNWInt("TA:DivventRotation") then weapon:SetNWInt("TA:DivventRotation", 0) end
	if not weapon:GetNWInt("TA:DivventIndex") or weapon:GetNWInt("TA:DivventIndex") == 0 then weapon:SetNWInt("TA:DivventIndex", 1) end
...
function TOOL.BuildCPanel(CPanel)
	...
	CPanel:AddControl("PropSelect", {ConVar = "aperture_diversity_vent_model", Models = list.Get("DiversityVentModels"), Height = 3})
	CPanel:NumSlider("#tool.aperture_diversity_vent.count", "aperture_diversity_vent_count", 1, 100, 0)
	CPanel:AddControl("CheckBox", {Label = "#tool.aperture_diversity_vent.ignorealive", Command = "aperture_diversity_vent_ignorealive", Help = 1})
	CPanel:AddControl("CheckBox", {Label = "#tool.aperture_diversity_vent.startenabled", Command = "aperture_diversity_vent_startenabled", Help = 1})
	CPanel:AddControl("Numpad", {Label = "#tool.aperture_diversity_vent.keyenable", Command = "aperture_diversity_vent_keyenable"})
