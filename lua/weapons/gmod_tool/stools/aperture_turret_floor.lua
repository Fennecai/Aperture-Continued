...
if CLIENT then
	...
	language.Add(
		"tool.aperture_turret_floor.desc",
		"A Turret will protect specific places from any hostile....even if they're friendly."
	)
...
if SERVER then
	...
	duplicator.RegisterEntityClass(
		"npc_portal_turret_floor_defective",
		MakePortalTurretFloor,
		"pos",
		"ang",
		"model",
...
function TOOL:LeftClick(trace)
	...
	if trace.HitNormal == UP then
		...
		if model == "models/aperture/rocket_sentry.mdl" then
			angle = angle - Angle(0, 90, 0)
		end
		angle = Angle(0, angle.y, 0)
		_,
			ang = LocalToWorld(Vector(), angle, Vector(), ang)
	end
...
function TOOL.BuildCPanel(cPanel)
	...
	cPanel:AddControl(
		"PropSelect",
		{ConVar = "aperture_turret_floor_model", Models = list.Get("PortalTurretFloorModels"), Height = 1}
	)
	cPanel:AddControl(
		"CheckBox",
		{Label = "#tool.aperture_turret_floor.startenabled", Command = "aperture_turret_floor_startenabled", Help = true}
	)
	cPanel:AddControl(
		"Numpad",
		{Label = "#tool.aperture_turret_floor.enable", Command = "aperture_turret_floor_keyenable"}
	)
