...
function LIB_APERTURECONTINUED:GetPaintInfo(startpos, dir, ignoreGelledProps, excludeNormalDifferents, sufraceNormalToCompare)
...
function LIB_APERTURECONTINUED:CreateNewPaintType(index, info)
	if not LIB_APERTURECONTINUED.PAINT_TYPES[index] then PORTAL_PAINT_COUNT = PORTAL_PAINT_COUNT + 1 end
...
local function ResolveWorldPaint(ply)
	...
	local paintType, paintNormal, paintHitPos = LIB_APERTURECONTINUED:GetPaintInfo(ply:GetPos() + orientation * playerHeight / 2, dir)
	...
	local paintTypeFront, paintNormalFront, paintHitPosFront = LIB_APERTURECONTINUED:GetPaintInfo(plyOrientCenter, moveDirection * playerWidth * 2)
	if not paintTypeFront then
		...
		paintTypeFront, paintNormalFront, paintHitPosFront = LIB_APERTURECONTINUED:GetPaintInfo(traceForwardFloorDown.HitPos, -moveDirection * playerWidth * 2)
	end
	...
	if LIB_APERTURECONTINUED:IsPlayerOnGround(ply) then
		...
		if hitWall.Hit then
			local paintType, paintNormal, paintHitPos = LIB_APERTURECONTINUED:GetPaintInfo(ply:GetPos() + Vector(0, 0, playerHeight), ply:GetVelocity():GetNormalized() * playerHeight)
...
hook.Add("OnPlayerHitGround", "TA:Paint_OnPlayerHitGround", function(ply, inwater, onFloater, speed)
	local paintType, paintNormal, paintHitPos = LIB_APERTURECONTINUED:GetPaintInfo(ply:GetPos(), ply:GetVelocity())
