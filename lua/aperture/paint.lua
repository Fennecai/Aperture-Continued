...
-- ================================ PAINT STUFF ============================
LIB_APERTURECONTINUED.PAINT_QUALITY = 1
LIB_APERTURECONTINUED.PAINT_TYPES = {}
...
LIB_APERTURECONTINUED.GEL_BOX_SIZE = 64
LIB_APERTURECONTINUED.GEL_MAXSIZE = 150
LIB_APERTURECONTINUED.GEL_MINSIZE = 40
LIB_APERTURECONTINUED.GEL_MAX_LAUNCH_SPEED = 1000

LIB_APERTURECONTINUED.GELLED_ENTITIES = {}
LIB_APERTURECONTINUED.CONNECTED_PAINTS = {}

local PLAYER_PAINT_CAMERA = {}

function LIB_APERTURECONTINUED:IsPlayerOnGround(ply)
	local orientation = ply:GetNWVector("TA:Orientation")
	return orientation and orientation ~= Vector(0, 0, 1) or ply:IsOnGround()
end

function LIB_APERTURECONTINUED:InvertNormal(normal)
	if normal.x ~= 0 then
		normal.x = normal.x * -1
	end
	if normal.y ~= 0 then
...
function LIB_APERTURECONTINUED:GetPaintInfo(
	startpos,
	dir,
	ignoreGelledProps,
	excludeNormalDifferents,
	sufraceNormalToCompare)
	if not LIB_PAINT then
		return
	end
	local paintInfo,
		point = LIB_PAINT:GetPaintInfo(startpos, dir)
	local trace =
		util.TraceLine(
		{
			start = startpos,
...
function LIB_APERTURECONTINUED:CreateNewPaintType(index, info)
	if not LIB_APERTURECONTINUED.PAINT_TYPES[index] then
		PORTAL_PAINT_COUNT = PORTAL_PAINT_COUNT + 1
	end
	LIB_APERTURECONTINUED.PAINT_TYPES[index] = info
end
...
function LIB_APERTURECONTINUED:PaintTypeToColor(paintType)
	return LIB_APERTURECONTINUED.PAINT_TYPES[paintType].COLOR
end

function LIB_APERTURECONTINUED:PaintTypeToName(index)
	return LIB_APERTURECONTINUED.PAINT_TYPES[index].NAME
end

function LIB_APERTURECONTINUED:PaintTypeToInfo(index)
	return LIB_APERTURECONTINUED.PAINT_TYPES[index]
end
...
function LIB_APERTURECONTINUED:PaintPlayerScreen(ply, paintType, radius)
	if CLIENT then
		return
	end
...
net.Receive(
	...
	function()
		...
		if not PLAYER_PAINT_CAMERA[ply] then
			PLAYER_PAINT_CAMERA[ply] = {}
		end
...
hook.Add(
	...
	function()
		...
		for k, v in pairs(paintCamInfos) do
			...
			local clr = LIB_APERTURECONTINUED:PaintTypeToColor(paintType)
			...
			surface.DrawTexturedRect(pos.x - rad / 2, pos.y - rad / 2, rad, rad * (1 + time / 10))

			v.pos.y = pos.y + math.max(0, time * 2 - 0.25)
			v.time = time + 0.2 * FrameTime()
			if time >= 1 then
				PLAYER_PAINT_CAMERA[ply][k] = nil
			end
		end
	end
)
...
function LIB_APERTURECONTINUED:PaintEntity(ent, paintType)
	local paint_model = ent:GetNWEntity("TA:PaintedModel")
	local paintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[paintType]

	if IsValid(paint_model) then
		local oldPaintType = ent:GetNWInt("TA:PaintType")
		local oldPaintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[oldPaintType]

		if
			oldPaintType ~= paintType and (oldPaintType and oldPaintType ~= PORTAL_PAINT_NONE) and
				(paintType and paintType ~= PORTAL_PAINT_NONE)
		 then
			if paintInfo.OnEntityChangedTo then
				paintInfo:OnEntityChangedTo(ent, paintType)
			end
			if oldPaintInfo.OnEntityChangedFrom then
				oldPaintInfo:OnEntityChangedFrom(ent, oldPaintType)
			end
		end

		ent:SetNWInt("TA:PaintType", paintType)
		paint_model:SetColor(LIB_APERTURECONTINUED:PaintTypeToColor(paintType))
		return
	end

	if paintInfo.OnEntityPainted then
		paintInfo:OnEntityPainted(ent)
	end

	paint_model = ents.Create("prop_physics")
	if not IsValid(paint_model) then
		return
	end
	...
	LIB_APERTURECONTINUED.GELLED_ENTITIES[ent] = v
end

function LIB_APERTURECONTINUED:ClearPaintedEntity(ent)
	local paint_model = ent:GetNWEntity("TA:PaintedModel")
	...
	local paintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[paintType]
	if paintInfo.OnEntityCleared then
		paintInfo:OnEntityCleared(ent)
	end

	LIB_APERTURECONTINUED.GELLED_ENTITIES[ent] = nil
	paint_model:Remove()
	ent:SetNWInt("TA:PaintType", 0)
end

function LIB_APERTURECONTINUED:MakePaintBlob(paintType, pos, velocity, radius)
	...
	if paintType == PORTAL_PAINT_WATER then
		ent:SetMaterial("models/aperture/water_blob")
	end
...
local function ResolveWorldPaint(ply)
	-- assigning variables
	if ply:GetNWVector("TA:Orientation") == Vector() then
		ply:SetNWVector("TA:Orientation", ORIENTATION_DEFAULT)
	end
	if not ply:GetNWVector("TA:OrientationWalk") then
		ply:SetNWVector("TA:OrientationWalk", Vector(0, 0, 0))
	end
	if not ply.TA_Player_PrevOrient then
		ply.TA_Player_PrevOrient = Angle()
	end
	if not ply.TA_LastTimeOnPaint then
	...
	if LIB_APERTURECONTINUED:IsPlayerOnGround(ply) or orientation ~= ORIENTATION_DEFAULT then
		dir = -orientation * (playerHeight + 50)
	else
		local velocity = ply:GetVelocity()
		dir = (Vector(0, 0, -0.5) + velocity:GetNormalized()) * velocity:Length()
		if dir:Length() > playerHeight then
			dir = dir:GetNormalized() * playerHeight
		end
	end
	local paintType,
		paintNormal,
		paintHitPos = LIB_APERTURECONTINUED:GetPaintInfo(ply:GetPos() + orientation * playerHeight / 2, dir)

	-- Getting player moving speed
	local speed = ply:GetWalkSpeed()
	if ply:KeyDown(IN_WALK) then
		speed = ply:GetWalkSpeed() / 2
	end
	...
	local _,
		localangle = WorldToLocal(Vector(), eyeAngle, Vector(), orientationAng)
	localangle = Angle(0, localangle.yaw, 0)
	local _,
		worldangle = LocalToWorld(Vector(), localangle, Vector(), orientationAng)
	...
	local paintTypeFront,
		paintNormalFront,
		paintHitPosFront = LIB_APERTURECONTINUED:GetPaintInfo(plyOrientCenter, moveDirection * playerWidth * 2)
	if not paintTypeFront then
		...
		paintTypeFront,
			paintNormalFront,
			paintHitPosFront = LIB_APERTURECONTINUED:GetPaintInfo(traceForwardFloorDown.HitPos, -moveDirection * playerWidth * 2)
	end
	...
	if not paintType and lastPaintType and (lastTimeOnPaint and CurTime() > lastTimeOnPaint + 0.1) then
		local prevPaintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[ply.TA_LastPaintType]

		if prevPaintInfo.OnExit then
			prevPaintInfo:OnExit(ply, ply.TA_LastPaintNormal)
		end
	...
	if LIB_APERTURECONTINUED:IsPlayerOnGround(ply) or paintTypeFront then
		local lastPaintType = ply.TA_LastPaintType
		local lastTimeOnPaint = ply.TA_LastTimeOnPaint

		if lastPaintType ~= paintType and paintType and lastPaintType then
			local prevPaintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[lastPaintType]
			if prevPaintInfo.OnChangeFrom then
				prevPaintInfo:OnChangeFrom(ply, paintType, paintNormal)
			end
			if paintInfo.OnChangeTo then
				paintInfo:OnChangeTo(ply, lastPaintType, paintNormal)
			end
		...
		if (not lastPaintType or lastPaintType == PORTAL_PAINT_NONE) and (not lastTimeOnPaint or lastTimeOnPaint == 0) then
			if paintInfo.OnEnter then
				paintInfo:OnEnter(ply, paintNormal, paintHitPos)
			end
			ply.TA_LastPaintType = paintType
	...
	if LIB_APERTURECONTINUED:IsPlayerOnGround(ply) then
		-- Paint Think
		if ply.TA_LastPaintType and ply.TA_LastTimeOnPaint then
			if paintInfo.Think then
		...
		if orientation ~= paintNormal then
			if paintInfo.OnChangingOrientation then
				paintInfo:OnChangingOrientation(ply, orientation, paintNormal)
			end
		end
	else
		local mins,
			maxs = ply:GetPhysicsObject():GetAABB()
		...
		if hitWall.Hit then
			local paintType,
				paintNormal,
				paintHitPos =
				LIB_APERTURECONTINUED:GetPaintInfo(
				ply:GetPos() + Vector(0, 0, playerHeight),
				ply:GetVelocity():GetNormalized() * playerHeight
			)

			if paintType then
				local paintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[paintType]
				if paintInfo.OnLand then
					paintInfo:OnLand(ply, paintNormal)
				end
			end
		end
	end
...
local function ResolvePaintedEntities(ent)
	...
	if not IsValid(ent) then
		LIB_APERTURECONTINUED.GELLED_ENTITIES[k] = nil
		return
	end

	-- skip if props is freezed or it is holding by the player
	if IsValid(ent:GetPhysicsObject()) and not ent:GetPhysicsObject():IsMotionEnabled() or ent:IsPlayerHolding() then
		return
	end
...
hook.Add(
	...
	function()
		...
		for k, v in pairs(LIB_APERTURECONTINUED.GELLED_ENTITIES) do
...
hook.Add(
	"PlayerFootstep",
	"TA:Paint_Footsteps",
	function(ply, pos, foot, lsound, volume)
		if not ply.TA_LastPaintType then
			return ply, pos, foot, lsound, volume
		end
		local orientation = ply:GetNWVector("TA:Orientation")
		local playerHeight = ply:GetModelRadius()
...
hook.Add(
	"KeyPress",
	"TA:Paint_ButtonPressed",
	function(ply, key)
		if not ply.TA_LastPaintType then
			return
		end
		local paintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[ply.TA_LastPaintType]

		if paintInfo.OnButtonPressed then
			paintInfo:OnButtonPressed(ply, ply.TA_LastPaintNormal, key)
		end
	end
)
...
hook.Add(
	"OnPlayerHitGround",
	"TA:Paint_OnPlayerHitGround",
	function(ply, inwater, onFloater, speed)
		local paintType,
			paintNormal,
			paintHitPos = LIB_APERTURECONTINUED:GetPaintInfo(ply:GetPos(), ply:GetVelocity())
		if not paintType then
			return ply, inwater, onFloater, 0
		end

		local paintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[paintType]
		if paintInfo.OnLanding then
			paintInfo:OnLanding(ply, paintNormal, speed)
		end
		return ply, inwater, onFloater, 0
	end
)
...
hook.Add(
	...
	function(ply, speed)
		...
		local paintInfo = LIB_APERTURECONTINUED.PAINT_TYPES[paintType]
		if paintInfo.OnGetFallDamage then
			return paintInfo:OnGetFallDamage(ply, speed)
		end
	end
)
