...
function LIB_APERTURECONTINUED:GetPortalRotateVector(vec, portal, flip)
	...
	if not portal:IsLinked() then
		return
	end
	local portalOther = portal:GetOther()
	if not IsValid(portalOther) then
		return
	end
...
function LIB_APERTURECONTINUED:GetPortalTransform(pos, ang, portal, flip)
	...
	local pang = flip and portalOther:LocalToWorldAngles(Angle(0, 180, 0)) or portalOther:GetAngles()
	pos,
		ang = WorldToLocal(pos, ang, portal:GetPos(), portal:GetAngles())
	...
	repeat
		...
		trace =
			util.TraceLine(
			{
				...
				filter = function(ent)
					if
						ent ~= ignore and not MathEntityAndTable(ent, ignore) and not ignoreEntities and ent:GetClass() ~= "prop_portal" and
							(not ignoreAlive or (ent:IsPlayer() or ent:IsNPC()))
					 then
						return true
					end
				end
		...
		for k, v in pairs(ents.FindByClass("prop_portal")) do
			if v ~= exitPortal then
				...
				if pos.x > -30 and pos.x < 10 and pos.y > -30 and pos.y < 30 and pos.z > -45 and pos.z < 45 then
		...
		passages = passages + 1
		if passages >= LIB_APERTURECONTINUED.MAX_PASSAGES then
			break
		end
	until hitPortal
...
local function RFindClosestAliveInSphereIncludingPortalPassages(entities, startpos, length, degrese, portal, distance)
	...
	for k, v in pairs(entities) do
		...
		if v:GetClass() == "prop_portal" and v ~= portal and v:IsLinked() then
			...
			if not degrese or deg < degrese then
				degrese = deg
			end
			...
			local e,
				p,
				d = RFindClosestAliveInSphereIncludingPortalPassages(entits, startp, length, degrese, portalOther, d)
		...
		if
			((v:IsPlayer() and v:Alive() and not LIB_APERTURECONTINUED:GetAIIgnorePlayers()) or v:IsNPC() and v:Health() > 0) and
				(dist == -1 or dist > d)
		 then
...
function OverridedShootPortal(self, type)
	...
	if IsFirstTimePredicted() and owner:IsValid() then --Predict that motha' fucka'
		if SERVER then
			...
			if
				(trace.Hit and
					(trace.HitWorld or trace.Entity and trace.Entity:GetClass() == "env_portal_wall" and trace.Entity:GetSkin() == 0))
			 then
				local validpos,
				...
				if
					not trace.HitNoDraw and not trace.HitSky and
...
hook.Add(
	"Think",
	"TA:PortalGunOverride",
	function()
		if SERVER then
			for k, v in pairs(player.GetAll()) do
				if
					IsValid(v:GetActiveWeapon()) and v:GetActiveWeapon():GetClass() == "weapon_portalgun" and TYPE_BLUE and TYPE_ORANGE
				 then
