...
if SERVER then
	function PlayerChangeOrient(ply, orientation, paintHitPos)
		...
		if not paintHitPos then
			_,
				_,
				paintHitPos = LIB_APERTURECONTINUED:GetPaintInfo(plyOrientCenter, -orientPlayerHeight * 1.5)
		end
		...
		timer.Create(
			"TA:Player_Changed" .. ply:EntIndex(),
			1,
			1,
			function()
			end
	...
	function PAINT_INFO:OnEnter(ply, normal, enterPos)
		local orientation = ply:GetNWVector("TA:Orientation")
		if LIB_MATH_TA:DegreeseBetween(normal, orientation) > 1 then
			PlayerChangeOrient(ply, normal, enterPos)
		end
	...
	function PAINT_INFO:Think(ply, normal, orientationMove)
		...
		local orientation = ply:GetNWVector("TA:Orientation")
		local orientPlayerHeight =
			ply:KeyDown(IN_DUCK) and orientation * playerHeightFull / 2 or orientation * playerHeightFull
		...
		if orienationWalk ~= Vector() and orientation ~= ORIENTATION_DEFAULT then
			...
			if not timer.Exists("TA:PlayerFootsteps" .. ply:EntIndex()) and orientationMove:Length() > 0 then
				...
				timer.Create(
					"TA:PlayerFootsteps" .. ply:EntIndex(),
					time,
					1,
					function()
					end
			...
			if
				not traceForwardFloor.Hit and not traceForwardFloorDown.Hit and traceForwardFloorBack.Hit and
					traceForwardFloorBack.Fraction > 0 and
					LIB_MATH_TA:DegreeseBetween(ORIENTATION_DEFAULT, traceForwardFloorBack.HitNormal) < 25 and
					LIB_MATH_TA:DegreeseBetween(ORIENTATION_DEFAULT, orientation) > 45
			 then
				-- Step on conner
				...
				if ply:KeyPressed(IN_JUMP) then
					...
					if traceForward.Hit then
						orientationMove = orientationMove - orientationAng:Forward() * (1 - traceForward.Fraction) * plyWidth
					end
					if traceBack.Hit then
						orientationMove = orientationMove + orientationAng:Forward() * (1 - traceBack.Fraction) * plyWidth
					end
					if traceRight.Hit then
						orientationMove = orientationMove - orientationAng:Right() * (1 - traceRight.Fraction) * plyWidth
					end
					if traceLeft.Hit then
						orientationMove = orientationMove + orientationAng:Right() * (1 - traceLeft.Fraction) * plyWidth
					end
...
if CLIENT then
	local function ChangeCamerOrientation(ply, angleFrom, angleTo, plyCamera)
		...
		local _,
			angOffset = WorldToLocal(Vector(), plyCamera, Vector(), angleFrom)
	...
	hook.Add(
		...
		function()
			...
			if not ply:GetNWVector("TA:PrevOrientation") then
				ply:SetNWVector("TA:PrevOrientation", Vector())
			end
			if not ply:GetNWAngle("TA:PrevOrientationAng") then
				ply:SetNWAngle("TA:PrevOrientationAng", Vector(0, 0, 1):Angle())
			end
			if not ply:GetNWAngle("TA:PlayerAng") then
				ply:SetNWAngle("TA:PlayerAng", eyeAngles)
			end
			if not ply:GetNWAngle("TA:PlayerEyeAngle") then
				ply:SetNWAngle("TA:PlayerEyeAngle", eyeAngles)
			end
			...
			if orientation ~= prevOrientation then
				local _,
					angleFrom = WorldToLocal(Vector(), (-orientation):Angle(), Vector(), prevOrientationAng)
				angleFrom = Angle(0, angleFrom.yaw, 0)
				_,
					angleFrom = LocalToWorld(Vector(), angleFrom, Vector(), prevOrientationAng)

				local _,
					angleTo = WorldToLocal(Vector(), prevOrientation:Angle(), Vector(), orientationAng)
				angleTo = Angle(0, angleTo.yaw, 0)
				_,
					angleTo = LocalToWorld(Vector(), angleTo, Vector(), orientationAng)
			...
			if newEyeAngle ~= eyeAngles then
				...
				if playerAng ~= eyeAngles then
					...
					if playerEyeAngle.y > 360 then
						playerEyeAngle.y = playerEyeAngle.y - 360
					end
					if playerEyeAngle.y < -360 then
						playerEyeAngle.y = playerEyeAngle.y + 360
					end
				...
				if ply.TA_Current_Ang and ply.TA_New_Ang and ply.TA_Camera_Ang_Offset then
					...
					local _,
						offsetAngle = WorldToLocal(Vector(), newAng, Vector(), currentAng)
					offsetAngle = offsetAngle * FrameTime() * 10
					_,
						offsetAngle = LocalToWorld(Vector(), offsetAngle, Vector(), currentAng)
					currentAng = offsetAngle
					ply.TA_Current_Ang = currentAng

					local _,
						camAng = LocalToWorld(Vector(), cameraAngOffset, Vector(), currentAng)
					...
					if result < 1 then
						...
						local _,
							camOffAng = LocalToWorld(Vector(), Angle(0, 0, 0), Vector(), newAng)
						ply:SetNWAngle("TA:PrevOrientationAng", camOffAng)
						ply:SetNWAngle("TA:PlayerEyeAngle", cameraAngOffset)
					end
				else
					_,
						newEyeAngle = LocalToWorld(Vector(), playerEyeAngle, Vector(), prevOrientationAng)

					local plyAng = -ply:GetAngles()
					local _,
						orientAngToPly = WorldToLocal(Vector(), plyAng, Vector(), prevOrientationAng)
			...
			if IsValid(collider) then
				if not IsValid(constraint.Find(ent, collider, "Weld", 0, 0)) then
					...
					timer.Simple(
						0,
						function()
							constraint.Weld(ent, collider, 0, 0, 3000, 1, false)
						end
					)
				...
				timer.Simple(
					0,
					function()
						constraint.Weld(ent, Entity(0), 0, 0, 10000, 1, false)
					end
				)
				ent:EmitSound("TA:PaintStickEnter")
			end
