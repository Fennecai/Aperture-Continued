...
if SERVER then
	...
	function PAINT_INFO:OnEnter(ply, normal)
		if LIB_MATH_TA:DegreeseBetween(normal, ORIENTATION_DEFAULT) > 35 then
			return
		end
	...
	function PAINT_INFO:OnChangeTo(ply, oldType, normal)
		if oldType == PORTAL_PAINT_STICKY then
			Bounce(ply, normal)
			return
		end
		if LIB_MATH_TA:DegreeseBetween(normal, ORIENTATION_DEFAULT) > 35 then
		...
		if oldType == PORTAL_PAINT_SPEED and ply:GetVelocity():Length() > 1000 then
			Bounce(ply, normal)
		end
	end
