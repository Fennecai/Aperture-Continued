...
function LIB_MATH_TA:ConvertToGrid(pos, size)
	local gridPos =
		Vector(math.Round(pos.x / size) * size, math.Round(pos.y / size) * size, math.Round(pos.z / size) * size)
...
function LIB_MATH_TA:AnglesToZeroz(angle)
	if math.abs(angle.p) < LIB_MATH_TA.EPSILON then
		angle.p = 0
	end
	if math.abs(angle.y) < LIB_MATH_TA.EPSILON then
		angle.y = 0
	end
	if math.abs(angle.r) < LIB_MATH_TA.EPSILON then
		angle.r = 0
	end
end
...
function LIB_MATH_TA:NormalFlipZeros(normal)
	if math.abs(normal.x) < LIB_MATH_TA.EPSILON then
		normal.x = 0
	end
	if math.abs(normal.y) < LIB_MATH_TA.EPSILON then
		normal.y = 0
	end
	if math.abs(normal.z) < LIB_MATH_TA.EPSILON then
		normal.z = 0
	end
end
