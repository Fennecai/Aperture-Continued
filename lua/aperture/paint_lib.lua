...
local function AngleToZeroz(angle)
	if math.abs(angle.p) < 0.0001 then
		angle.p = 0
	end
	if math.abs(angle.y) < 0.0001 then
		angle.y = 0
	end
	if math.abs(angle.r) < 0.0001 then
		angle.r = 0
	end
end
...
local function NormalFlipZeros(normal)
	...
	if math.abs(normal.x) < lower then
		normal.x = 0
	end
	if math.abs(normal.y) < lower then
		normal.y = 0
	end
	if math.abs(normal.z) < lower then
		normal.z = 0
	end
end
...
local function ConvertToGridOnSurface(pos, angle, radius, zRound)
	...
	if zRound == 0 then
		...
		WTL =
			Vector(math.Round(WTL.x / radius) * radius, math.Round(WTL.y / radius) * radius, math.Round(WTL.z / zRound) * zRound)
	end
...
function LIB_PAINT:GetCellPaintInfo(pos)
	if
		not LIB_PAINT.PAINT_INFO[pos.x] or not LIB_PAINT.PAINT_INFO[pos.x][pos.y] or
			not LIB_PAINT.PAINT_INFO[pos.x][pos.y][pos.z]
	 then
		return nil
	end
...
function LIB_PAINT:GetPaintInfo(start, direction)
	local trace =
		util.TraceLine(
		{
			...
			filter = function(ent)
				if ent:GetClass() == "env_portal_wall" then
					return true
				end
			end
		}
...
local function CheckForPossiblePlacement(uvX, uvY, chunkPos, paintPos, offset, normal)
	...
	local trace =
		util.TraceLine(
		{
			...
			collisiongroup = COLLISION_GROUP_DEBRIS,
			filter = function(ent)
				if IsValid(ent) and ent:GetClass() == "env_portal_wall" then
					return true
				end
			end
		}
	...
	for x = -radiusDivided, radiusDivided do
		for y = -radiusDivided, radiusDivided do
			if Vector(x, y):Distance(Vector(0, 0)) < radiusDivided * math.Rand(paintDat.hardness, 1) then
				local uvX,
					uvY = math.Round(UVpos.y + x), math.Round(UVpos.x + y)
				if uvX >= 0 and uvY >= 0 and uvX <= TEXTURE_SIZE and uvY <= TEXTURE_SIZE then
					...
					local wX,
						wY = x * chunkTextRatio, y * chunkTextRatio
					...
					if not paintInfo or paintInfo.paintType ~= paintDat.paintType then
						if not clear then
							if not LIB_PAINT.PAINT_INFO[cellPos.x] then
								LIB_PAINT.PAINT_INFO[cellPos.x] = {}
							end
							if not LIB_PAINT.PAINT_INFO[cellPos.x][cellPos.y] then
								LIB_PAINT.PAINT_INFO[cellPos.x][cellPos.y] = {}
							end
						end
...
local function PaintChunk(pos, chunkPos, paintPos, normal, paintDat, clear)
	local normalAngle = normal:Angle() + Angle(90, 0, 0)
	AngleToZeroz(normalAngle)
	if chunkPos == nil then
		chunkPos = ConvertToGridOnSurface(pos, normalAngle, CHUNK_RADIUS, CHUNK_THINKNESS)
	end
	if paintPos == nil then
		paintPos = ConvertToGridOnSurface(pos, normalAngle, CHUNK_RADIUS, 0)
	end
...
function LIB_PAINT:InitPaint()
	...
	local genBumpTexture =
		GetRenderTargetEx(
		"paint_bubbles_normal",
		genBumpmapSize,
		genBumpmapSize,
...
hook.Add(
	"Initialize",
	"ApertureContinued:InitializePaint",
	function()
		LIB_PAINT:InitPaint()
	end
)
...
-- if not timer.Exists("PlyPaint"..k) and (v:KeyDown(IN_RELOAD) or v:KeyDown(IN_USE)) then
