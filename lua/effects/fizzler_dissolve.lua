function EFFECT:Init(data)
	...
	if (not self.eEntity:IsValid()) then
		return
	end
	...
	local min,
		max = self.eEntity:GetModelBounds()
	local randVec = Vector(math.Rand(min.x, max.x), math.Rand(min.y, max.y), math.Rand(min.z, max.z))

	for count = 1, 2 do
		local p =
			self.Emitter:Add(
			"sprites/gmdm_pickups/light",
			self.eEntity:LocalToWorld(VectorRand() * randVec + self.eEntity:OBBCenter())
		)
