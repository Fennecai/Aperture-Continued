...
function EFFECT:Init(data)
	...
	local borderSize = IsValid(entity) and math.cos((math.abs(localpos.z) / PORTAL_HEIGHT_HALF) * math.pi / 2) * BRIDGE_WIDTH or BRIDGE_WIDTH
	...
	for i = -1,1,0.1 do
		...
		if not IsValid(entity) or (i * BRIDGE_WIDTH <= (-borderSize + localpos.y) or i * BRIDGE_WIDTH >= (borderSize + localpos.y)) then
